...
class SwerveOdometry(Node):
    ...
    # Must stay identical to WHEEL_GEOMETRY in swerve_drive_controller.py.
    ...
    # Must stay identical to WHEEL_ROLL_SIGN in swerve_drive_controller.py.
    # The URDF's wheel drive-joint <axis> is mirrored left/right (FL/BL =
    # "0 -1 0", FR/BR = "0 1 0"), so raw joint velocity must be sign-
    # corrected before it represents true forward wheel speed.
    WHEEL_ROLL_SIGN = {
        'FL_wheel_joint': -1.0,
        'FR_wheel_joint': 1.0,
        'BL_wheel_joint': -1.0,
    ...
    def joint_state_callback(self, msg: DynamicJointState):
        ...
        for steer_joint, drive_joint in self.STEER_TO_DRIVE.items():
            ...
            wheel_linear_speed = (
                self.WHEEL_ROLL_SIGN[drive_joint] * wheel_ang_vel * self.wheel_radius
            )
