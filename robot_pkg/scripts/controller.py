...
class SwerveDriveController(Node):
    ...
    # Wheel (x, y) offsets in the base_link frame at neutral (0 rad)
    # suspension deflection, derived from the URDF kinematic chain
    # (suspension_joint origin -> mount_joint origin -> wheel_joint origin).
    # NOTE: the front/rear x values are NOT symmetric (+0.3714 / -0.5186)
    # because the suspension pivot is offset from the chassis center by
    # -0.0735645 m in x. Do not "round" these back to +-0.445.
    WHEEL_GEOMETRY = {
        'FL_wheel_joint': (0.3714, 0.369),    # front-left
        'FR_wheel_joint': (0.3714, -0.369),   # front-right
        'BL_wheel_joint': (-0.5186, 0.369),   # rear-left
        'BR_wheel_joint': (-0.5186, -0.369),  # rear-right
    }
    ...
    # The URDF's wheel drive-joint <axis> is mirrored left/right
    # (FL/BL = "0 -1 0", FR/BR = "0 1 0"), since left and right wheels
    # are physically mirrored parts. A positive joint velocity command
    # therefore spins left and right wheels in OPPOSITE physical
    # directions unless compensated here. +1 for a +Y axis wheel,
    # -1 for a -Y axis wheel. Must stay identical to WHEEL_ROLL_SIGN
    # in swerve_odometry.py.
    WHEEL_ROLL_SIGN = {
        'FL_wheel_joint': -1.0,
        'FR_wheel_joint': 1.0,
        'BL_wheel_joint': -1.0,
    ...
    def cmd_vel_callback(self, msg: Twist):
        ...
        for joint in self.JOINT_ORDER:
            ...
            # m/s -> rad/s, with sign flipped for wheels whose drive-joint
            # axis points the opposite way (see WHEEL_ROLL_SIGN above).
            raw_wheel_vel = self.WHEEL_ROLL_SIGN[joint] * speed / self.WHEEL_RADIUS
            velocity_cmds.append(raw_wheel_vel)
